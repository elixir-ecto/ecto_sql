defmodule Ecto.TypeTest do
  ...
  import Ecto.Type
  alias Ecto.Adapters.{MyXQL, Postgres, MsSql}
  ...
  test "loads through the adapter" do
    ...
    assert adapter_load(MsSql, {:map, Tds.Types.UUID}, %{"a" => @mssql_uuid_binary}) ==
           {:ok, %{"a" => @uuid_string}}
    ...
    assert adapter_load(MsSql, {:map, Tds.Types.UUID}, %{"a" => @uuid_string}) ==
           {:ok, %{"a" => @uuid_string}}
  end
end
