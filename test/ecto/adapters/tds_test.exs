defmodule Ecto.Adapters.TdsTest do
  ...
  test "aliasing a selected value with alias/2" do
  ...
  test "raises if alias/2 is used in a subquery" do
    message = ~r"`alias/2` can only be used in the outer most `select` expression."

    assert_raise ArgumentError, message, fn ->
      subquery = "schema" |> select([s], %{x: alias(s.x, :integer)}) |> subquery() |> plan()
      all(subquery)
    end
  end

  test "group_by can reference the alias of a selected value with alias/1" do
    query = "schema" |> select([s], alias(s.x, :integer)) |> group_by(alias(:integer)) |> plan()
    assert all(query) == ~s{SELECT s0.[x] AS integer FROM [schema] AS s0 GROUP BY integer}
  end

  test "order_by can reference the alias of a selected value with alias/1s" do
    query = "schema" |> select([s], alias(s.x, :integer)) |> order_by(alias(:integer)) |> plan()
    assert all(query) == ~s{SELECT s0.[x] AS integer FROM [schema] AS s0 ORDER BY integer}

    query = "schema" |> select([s], alias(s.x, :integer)) |> order_by([desc: alias(:integer)]) |> plan()
