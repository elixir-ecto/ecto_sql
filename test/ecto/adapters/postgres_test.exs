defmodule Ecto.Adapters.PostgresTest do
  ...
  test "where x and in subquery" do
    q = from(s in Schema, where: s.x in ^[2], select: s.z)
  ...
  test "where in subquery and x" do
    q = from(s in Schema, where: s.x in ^[1], select: s.z)
    s = from(s in Schema, where: s.z in subquery(q) and s.y > ^2, select: count())

    {q, params} = Ecto.Adapter.Queryable.plan_query(:all, Ecto.Adapters.Postgres, s)

    assert ~s{SELECT count(*) FROM "schema" AS s0 WHERE (s0."z" IN (SELECT s0."z" AS "z" FROM "schema" AS s0 WHERE (s0."x" = ANY($1))) AND (s0."y" > $2))} = all(q)
    assert params == [[1], 2]
  end
