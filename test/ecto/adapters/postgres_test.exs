defmodule Ecto.Adapters.PostgresTest do
  ...
  test "create table with identity key and references" do
    create = {:create, table(:posts),
              [{:add, :id, :identity, [primary_key: true]},
               ...
               {:add, :category_1, %Reference{table: :categories, type: :identity}, [identity_type: "int"]},
               {:add, :name, :string, []}]}

    assert execute_ddl(create) == ["""
    CREATE TABLE "posts" ("id" bigint GENERATED BY DEFAULT AS IDENTITY,
    "category_0" bigint CONSTRAINT "posts_category_0_fkey" REFERENCES "categories"("id"),
    "category_1" int CONSTRAINT "posts_category_1_fkey" REFERENCES "categories"("id"),
    "name" varchar(255),
    PRIMARY KEY ("id"))
    """ |> remove_newlines]
  end
  ...
  test "create table with int identity key" do
    # cannot use Application.put_env(:ecto_sql, :postgres_identity_type, "int")
    # without changing it for all concurrent tests
    create = {:create, table(:posts),
              [{:add, :id, :identity, [primary_key: true, identity_type: :int]}]}

    assert execute_ddl(create) == ["""
    CREATE TABLE "posts" ("id" int GENERATED BY DEFAULT AS IDENTITY, PRIMARY KEY ("id"))
    """ |> remove_newlines]
  end
