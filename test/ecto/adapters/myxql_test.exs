defmodule Ecto.Adapters.MyXQLTest do
  ...
  defp plan(query, operation \\ :all) do
    {query, _cast_params, _dump_params} =
      Ecto.Adapter.Queryable.plan_query(operation, Ecto.Adapters.MyXQL, query)
  ...
  test "from with hints" do
    query =
      Schema |> from(hints: ["USE INDEX FOO", "USE INDEX BAR"]) |> select([r], r.x) |> plan()
  ...
  test "from without schema" do
    ...
    assert_raise Ecto.QueryError,
                 ~r"MySQL adapter does not support selecting all fields from `posts` without a schema",
                 fn ->
  ...
  test "from with fragment" do
    ...
    assert_raise Ecto.QueryError,
                 ~r"MySQL adapter does not support selecting all fields from fragment",
                 fn ->
  ...
  test "aggregate filters" do
    ...
    assert_raise Ecto.QueryError,
                 ~r/MySQL adapter does not support aggregate filters in query/,
                 fn ->
                   all(query)
                 end
  end
  ...
  test "having can reference the alias of a selected value with selected_as/1" do
    ...
    assert all(query) ==
             ~s{SELECT s0.`x` AS `integer` FROM `schema` AS s0 GROUP BY `integer` HAVING (`integer` > 0)}
  end
  ...
  describe "windows" do
    ...
    test "partition by and order by on over" do
      query = select(Schema, [r], count(r.x) |> over(partition_by: [r.x, r.z], order_by: r.x))
  ...
  test "insert with on duplicate key" do
    ...
    assert_raise ArgumentError,
                 ":conflict_target is not supported in insert/insert_all by MySQL",
                 fn ->
  ...
  test "insert with query" do
    ...
    assert query ==
             ~s{INSERT INTO `schema` (`x`,`y`,`z`) VALUES (?,(SELECT s0.`id` FROM `schema` AS s0),?),(DEFAULT,DEFAULT,(SELECT s0.`id` FROM `schema` AS s0))}
  end
  ...
  test "create table with engine" do
    create =
      {:create, table(:posts, engine: :myisam), [{:add, :id, :serial, [primary_key: true]}]}
  ...
  test "create table with both engine and options" do
    create =
      {:create, table(:posts, engine: :myisam, options: "WITH FOO=BAR"),
       [{:add, :id, :serial, [primary_key: true]}, {:add, :created_at, :datetime, []}]}
      ...
      {:alter, table(:posts, comment: "table comment"),
       [
         ...
         {:modify, :permalink_id, %Reference{table: :permalinks},
          [null: false, comment: "column comment 2"]},
         {:remove, :summary}
       ]}
  ...
  test "rename index" do
    ...
    assert execute_ddl(rename) == [
             ~s|ALTER TABLE `people` RENAME INDEX `persons_name_index` TO `people_name_index`|
           ]
  end
