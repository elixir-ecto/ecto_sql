if Code.ensure_loaded?(Postgrex) do
  defmodule Ecto.Adapters.Postgres.Connection do
    ...
    defp column_type(type, opts) do
      ...
      # handle when user has defined a column like
      # add :id, :"bigint GENERATED BY DEFAULT AS IDENTITY"
      type_name =
        if type_name |> String.downcase() |> String.contains?(" generated") do
          [actual_type, _] = String.split(type_name, " ", parts: 2)
          actual_type
        else
          type_name
        end
    ...
    defp reference_column_type(:identity, opts), do: Keyword.get(opts, :identity_type, :bigint) |> ecto_to_db()
